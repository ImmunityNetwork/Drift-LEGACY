...
module.exports.run = async (bot, message, args) => {
    ...
    console.log(reason);
    if(!message.guild.member(message.author.user).hasPermmision(BAN_MEMBERS)) return message.reply("You dont have permmision to do that").then(message => message.delete(60000));
    ...
    if(!reason) {
        ...
        message.guild.member(user).sendMessage(`You have been unbanned by ${message.author.username}#${message.author.discriminator} due to ${reason}`);
}
module.exports.help = {
    name: "unban"
}
